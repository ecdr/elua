...
// Platform includes
// Has to be after hw_gpio.h because it overrides some values
...
#endif
#ifdef INT_GPIOJ
                                   , 0  // Space filler since there is no port I
                                   , INT_GPIOJ
#endif
#ifdef INT_GPIOK
                                   , INT_GPIOK
...
static const u8 timer_int_ids[] = { INT_TIMER0A, INT_TIMER1A, INT_TIMER2A, INT_TIMER3A, 
                                    INT_TIMER4A, INT_TIMER5A, INT_TIMER6A, INT_TIMER7A };

static const u8 uart_int_ids[] = { INT_UART0, INT_UART1, INT_UART2, INT_UART3, 
                                    INT_UART4, INT_UART5, INT_UART6, INT_UART7 };

#else

static const u8 timer_int_ids[] = { INT_TIMER0A, INT_TIMER1A, INT_TIMER2A, 
                                    INT_TIMER3A, INT_TIMER4A, INT_TIMER5A 
                                    // INT_WTIMER0A, INT_WTIMER1A, INT_WTIMER2A, 
                                    // INT_WTIMER3A, INT_WTIMER4A, INT_WTIMER5A
                                    };

static const u8 uart_int_ids[] = { INT_UART0, INT_UART1, INT_UART2, INT_UART3, 
                                    INT_UART4, INT_UART5, INT_UART7, INT_UART6 };
...
// Note that numeric arguments match the letter number (i.e., leave gaps for ports that are not used)

// TODO: Remove extra code for ports that are not part of build (#ifdef, etc.) - have to coordinate startup_gcc vectors
//  Makes startup_gcc even more of a tangled mess - consider can make startup_gcc neat (e.g. weak declarations)
void gpioa_handler()
{
...
void gpioj_handler()
{
  gpio_common_handler( 9 );
}

void gpiok_handler()
{
  gpio_common_handler( 10 );
}

void gpiol_handler()
{
  gpio_common_handler( 11 );
}

void gpiom_handler()
{
  gpio_common_handler( 12 );
}

void gpion_handler()
{
  gpio_common_handler( 13 );
}

void gpiop_handler()
{
  gpio_common_handler( 15 );
}

void gpioq_handler()
{
  gpio_common_handler( 16 );
}
// #endif
